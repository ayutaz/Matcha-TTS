...
def generate_sample(
    model, vocoder, text: str, spk_id: int,
    n_timesteps: int = 10, temperature: float = 0.667,
    length_scale: float = 1.0, clamp_boundary_blanks: bool = True,
    device: str = "cpu",
) -> dict:
...
def main(argv=None):
    ...
    parser.add_argument("--max-speakers", type=int, default=None,
                       help="Limit number of speakers (for testing)")
    ...
    for spk_id in tqdm(speakers, desc="Speakers"):
        ...
        for text_idx, text in enumerate(texts):
            try:
                ...
                dur_path.write_text(
                    json.dumps(dur_info, ensure_ascii=False, indent=2), encoding="utf-8")
