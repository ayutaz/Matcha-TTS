...
> **実装状況（2026-07-07更新）**: 本調査候補は9エージェントのワークフロー（分析→敵対的検証→統合）で
> 実コードに落とし込み済み。採否・実装状況・インスタンス起動後の手順は
> **`docs/training-speedup-implementation-plan.md`** を参照。要約: bf16恒久化=適用済み、
> A-1/C-1/C-2/A-2 scaffolding=実装済み（全てオプトイン・デフォルトOFF）、A-3=見送り、B-1=後回し。
