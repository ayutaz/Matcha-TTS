...
> **必読**: インスタンス選定・構築で実際に踏んだ問題（GFW経由の偽装ロケーションホスト、
> SSH鍵注入不良、Julius空.lab問題等）と対策は [vastai-troubleshooting.md](vastai-troubleshooting.md) にまとめてある。
> 新規インスタンスを立てる前に冒頭のチェックリストを確認すること。
