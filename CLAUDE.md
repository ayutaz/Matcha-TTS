...
## 音質改善の検証結果と恒久要件（2026-07、実測ベース）

音質（特に「濁り」）改善のための施策を実測で検証した結果。**以下は結論であり、再検証なしに覆さないこと**。
詳細は `docs/tsukuyomi-tts-verification-log.md`。

### R-1. fmaxは8000のまま維持する（引き上げ禁止）
- fmax=11025 は**客観的には高域(8-11kHz)を独立して改善する**（交絡排除A/B: 同データ・同ステップでfmaxのみ変更。
  高域エネルギー比 0.553→0.851、高域log-STFT L1で40/40サンプル勝利）。データ変更では埋まらないfmax固有の効果
- **しかしユーザ試聴では体感差ゼロ**。体感差のない改善のために破壊的変更（mel統計再計算・全前処理やり直し・
  音響モデル全再学習）を払う価値はないと判断 → **fmax=8000維持**
- 帰結: precompute の `--fmax` は既定8000のまま。JVS mel統計（-6.550095 / 2.383771）と既存資産の互換を保つ

### R-2. UTMOSを唯一の評価指標にしない（測れない領域がある）
- **UTMOS/SpeechMOSは内部で16kHzにダウンサンプルする** → **8kHz超の差を原理的に評価できない**。
  fmax=8000 vs 11025 のようなナイキスト付近の変更をUTMOSで判定してはならない
- **UTMOSはゲーム/アニメ音声を構造的に過小評価する**（ドメインバイアス）。キャラクター音声の評価には使わない
- 高域を測るには `scripts/eval_highband_fidelity.py`（8-11kHz log-STFT L1 + エネルギー比、GT基準）を使う
- **最終判断は必ず試聴で行う**（paired deltaと試聴を決め手にし、UTMOS単独では決めない）

### R-3. 濁りはボコーダでは取れない（ボコーダ交換で解決しようとしない）
- 独立2実験で確定: ①BigVGAN含む複数ボコーダのゼロショット差し替え（全て同等）、②fmax変更・日本語データ学習
- 濁りの主因はボコーダの種類ではなく、**音響モデルの予測mel側 or コーパスの本質的特性**にある

### R-4. ボコーダは「鳴らす音声のドメイン」に合わせる（言語一致では不十分）
- MoeSpeech（日本語キャラクター音声）で学習したWaveNeXtは、**JVS（日本語朗読）で汎用ゼロショットに負けた**
  （UTMOS: 汎用BSC 3.049 > HiFi-GAN 2.945 > 日本語学習 2.586。耳とUTMOS一致）
- **「同じ言語」≠「同じ音響ドメイン」**。汎用ボコーダはドメイン非依存で頑健
- 帰結: `checkpoints/wavenext_ja_8000_50kbatch.bin`（MoeSpeech学習）はJVS/朗読系には使わない。
  ボコーダを特化学習するなら、**最終的に鳴らす話者・ドメインの音声で**学習すること

### R-5. 単一話者fine-tuneでは話者embeddingが動いたか必ず測る
- つくよみfine-tune（jvs_aligned土台 100→101話者、90発話・33分）で、**学習後の話者embeddingが初期値
  （全話者平均）からほぼ動かなかった**: `cos=1.000, L2=0.031` → 声質が寄らず「平均声」のまま
- **真因は未特定**（EMA decay=0.9995の平滑化 / 64次元FiLMの弱いレバー / 小データ / 平均initが低勾配、等が候補）。
  **安易に原因を断定しないこと**。特に「detachが原因」は誤り — `text_encoder.py` L469/L473-474 と
  `matcha_tts.py` L270 を読むと、spk_emb には3つの生きた勾配経路がある（cat→proj_m→prior/diff loss、
  proj_w の FiLM→dur loss、decoder.compute_loss→diff loss）。detach(L473)が止めるのはDPのフレーム特徴xだけ
- **要件**: 単一話者fine-tune後は `spk_emb.weight[new_slot]` を初期値と比較（cos/L2）し、動いていなければ
  レシピ（EMA/LR/init話者/データ量）を疑う。動かないまま学習量を増やしても声質は寄らない
